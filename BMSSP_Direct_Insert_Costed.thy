...
text \<open>
  The following parallel relation is a conservative migration target for the
  stronger JAR-style accounting.  It keeps the established charged range slice
  used by the recurrence, but also records the semantic output returned by the
  child call.  Existing runs refine to it by choosing the semantic child output
  to be the charged slice; future strengthened runs can relax that equality
  without changing the old costed relation in place.
\<close>

inductive charged_direct_insert_costed_partition_loop_state
  and charged_direct_insert_costed_bmssp where
  Charged_Direct_Insert_State_Done:
    "keys_of D = {} \<Longrightarrow>
     bound_le (Fin a) B \<Longrightarrow>
     complete_on d' (bound_tree P B) \<Longrightarrow>
     charged_direct_insert_costed_partition_loop_state \<Delta> M_of t h k cap l d P B d' D a
       [] [] B [range_tree P a B] []
       (bound_tree P (Fin a) \<union> \<Union>(set [range_tree P a B])) 0 []"
| Charged_Direct_Insert_State_Stop:
    "bound_le (Fin a) B \<Longrightarrow>
     complete_on d' (bound_tree P (Fin a)) \<Longrightarrow>
     k * cap \<le> card (bound_tree P (Fin a)) \<Longrightarrow>
     charged_direct_insert_costed_partition_loop_state \<Delta> M_of t h k cap l d P B d' D a
       [] [] (Fin a) [range_tree P a (Fin a)] []
       (bound_tree P (Fin a) \<union> \<Union>(set [range_tree P a (Fin a)])) 0 []"
| Charged_Direct_Insert_State_Step:
    "pull_separates D (M_of l) Bmax S_pull beta D_pull \<Longrightarrow>
     bound_le (Fin beta) B \<Longrightarrow>
     bmssp_pre_full d S_pull (Fin beta) \<Longrightarrow>
     S_pull = split_below d P beta \<Longrightarrow>
     ...
     charged_direct_insert_costed_bmssp \<Delta> M_of t h k cap l d S_pull (Fin beta)
       d_child (Fin b) U_child c_child \<Longrightarrow>
     charged_child = range_tree P a (Fin b) \<Longrightarrow>
     complete_preserved d_child d' U_child \<Longrightarrow>
     direct_edge_batch =
       edge_relaxation_pairs_in_bound d_child charged_child beta B \<Longrightarrow>
     lower_edge_batch =
       edge_relaxation_pairs_between d_child charged_child b beta \<Longrightarrow>
     source_batch = label_pairs_between d S_pull b beta \<Longrightarrow>
     batch = direct_edge_batch @ lower_edge_batch @ source_batch \<Longrightarrow>
     D_next = batch_min_update D_pull batch \<Longrightarrow>
     partition_pull_cost_bound c_pull S_pull \<Longrightarrow>
     ...
     partition_batch_cost_bound c_lower h lower_edge_batch \<Longrightarrow>
     partition_batch_cost_bound c_sources h source_batch \<Longrightarrow>
     charged_direct_insert_costed_partition_loop_state \<Delta> M_of t h k cap l d P B d'
       D_next b betas bs B' charged_tail child_outputs_tail U_tail c_tail
       child_costs_tail \<Longrightarrow>
     c = c_pull + c_direct + c_lower + c_sources + c_child + c_tail \<Longrightarrow>
     charged_direct_insert_costed_partition_loop_state \<Delta> M_of t h k cap l d P B d' D a
       (beta # betas) (b # bs) B'
       (charged_child # charged_tail) (U_child # child_outputs_tail)
       (bound_tree P (Fin a) \<union> \<Union>(set (charged_child # charged_tail))) c
       (c_child # child_costs_tail)"
| Charged_Direct_Insert_Base:
    "S = {x} \<Longrightarrow>
...
| Charged_Direct_Insert_Step:
    "D = label_partition_view
     ...
     charged_direct_insert_costed_partition_loop_state \<Delta> M_of t h k cap l
       (find_pivots_label_capped k cap d S B)
       (find_pivots_pivots_capped k cap d S B) B d' D a betas bs B'
       charged_Us child_outputs U_loop c_loop child_costs_loop \<Longrightarrow>
     complete_on d'
       {v \<in> bound_tree S B'. find_pivots_label_capped k cap d S B v = dist s v} \<Longrightarrow>
     U = U_loop \<union>
       {v \<in> bound_tree S B'. find_pivots_label_capped k cap d S B v = dist s v} \<Longrightarrow>
...
lemma charged_direct_insert_partition_loop_state_step_done_exists:
  fixes a b beta Bmax :: real
    and B :: bound
    and D D_pull D_next :: "'a partition_view"
    and M_of :: "nat \<Rightarrow> nat"
    and \<Delta> t h k cap l c_pull c_direct c_lower c_sources c_child :: nat
    and d d' d_child :: "'a \<Rightarrow> real"
    and P S_pull U_child :: "'a set"
  assumes pull: "pull_separates D (M_of l) Bmax S_pull beta D_pull"
    ...
    and complete_a: "complete_on d' (bound_tree P (Fin a))"
    ...
    and c_pull: "partition_pull_cost_bound c_pull S_pull"
    and c_direct:
      "partition_batch_cost_bound c_direct t
        (edge_relaxation_pairs_in_bound d_child (range_tree P a (Fin b)) beta B)"
    and c_lower:
      "partition_batch_cost_bound c_lower h
        (edge_relaxation_pairs_between d_child (range_tree P a (Fin b)) b beta)"
    and c_sources:
      "partition_batch_cost_bound c_sources h (label_pairs_between d S_pull b beta)"
    and tail_empty: "keys_of D_next = {}"
    and b_bound: "bound_le (Fin b) B"
    and complete_B: "complete_on d' (bound_tree P B)"
  shows
    "charged_direct_insert_costed_partition_loop_state \<Delta> M_of t h k cap l d P B
      d' D a [beta] [b] B [range_tree P a (Fin b), range_tree P b B]
      [U_child]
      (bound_tree P (Fin a) \<union>
        \<Union>(set [range_tree P a (Fin b), range_tree P b B]))
      (c_pull + c_direct + c_lower + c_sources + c_child + 0) [c_child]"
proof -
  have tail:
    "charged_direct_insert_costed_partition_loop_state \<Delta> M_of t h k cap l d P B
      d' D_next b [] [] B [range_tree P b B] []
      (bound_tree P (Fin b) \<union> \<Union>(set [range_tree P b B])) 0 []"
    by (rule Charged_Direct_Insert_State_Done)
      (use tail_empty b_bound complete_B in auto)
  show ?thesis
    by (rule Charged_Direct_Insert_State_Step
        [where D_pull = D_pull
          and direct_edge_batch =
            "edge_relaxation_pairs_in_bound d_child (range_tree P a (Fin b)) beta B"
          and lower_edge_batch =
            "edge_relaxation_pairs_between d_child (range_tree P a (Fin b)) b beta"
          ...
          and batch =
            "edge_relaxation_pairs_in_bound d_child (range_tree P a (Fin b)) beta B @
             ...
             label_pairs_between d S_pull b beta"
          and D_next = D_next])
      (use pull beta_bound pre split reaches a_le_b a_bound complete_a small child
        preserved D_next c_pull c_direct c_lower c_sources tail in auto)
qed

theorem direct_insert_costed_refines_charged_direct_insert:
  "direct_insert_costed_partition_loop_state \<Delta> M_of t h k cap l d P B d' D a
     betas bs B' Us U c child_costs \<Longrightarrow>
   charged_direct_insert_costed_partition_loop_state \<Delta> M_of t h k cap l d P B d' D a
     betas bs B' Us (range_tree_child_list P a bs) U c child_costs"
and direct_insert_costed_bmssp_refines_charged_direct_insert:
...
proof (induction rule:
  ...
  have run:
    "charged_direct_insert_costed_partition_loop_state \<Delta> M_of t h k cap l d P
      B d' D a [] [] B [range_tree P a B] []
      (bound_tree P (Fin a) \<union> \<Union>(set [range_tree P a B])) 0 []"
    by (rule Charged_Direct_Insert_State_Done)
      (use Direct_Insert_Costed_State_Done in auto)
  then show ?case
    by simp
next
  ...
  have run:
    "charged_direct_insert_costed_partition_loop_state \<Delta> M_of t h k cap l d P
      B d' D a [] [] (Fin a) [range_tree P a (Fin a)] []
      (bound_tree P (Fin a) \<union> \<Union>(set [range_tree P a (Fin a)])) 0 []"
    by (rule Charged_Direct_Insert_State_Stop)
      (use Direct_Insert_Costed_State_Stop in auto)
  then show ?case
    by simp
next
  case (Direct_Insert_Costed_State_Step D M_of l Bmax S_pull beta D_pull B d
      P a b B' d' k cap \<Delta> t h d_child U_child c_child direct_edge_batch
      lower_edge_batch source_batch batch D_next c_pull c_direct c_lower
      c_sources betas bs Us_tail U_tail c_tail child_costs_tail c)
  have U_child_eq: "U_child = range_tree P a (Fin b)"
    using Direct_Insert_Costed_State_Step by blast
  have run:
    "charged_direct_insert_costed_partition_loop_state \<Delta> M_of t h k cap l d P B d' D a
      (beta # betas) (b # bs) B'
      (U_child # Us_tail) (U_child # range_tree_child_list P b bs)
      (bound_tree P (Fin a) \<union> \<Union>(set (U_child # Us_tail)))
      (c_pull + c_direct + c_lower + c_sources + c_child + c_tail)
      (c_child # child_costs_tail)"
    by (rule Charged_Direct_Insert_State_Step
      [where batch = batch and D_next = D_next and charged_child = U_child])
      (use Direct_Insert_Costed_State_Step in auto)
  then show ?case
    using U_child_eq Direct_Insert_Costed_State_Step by simp
next
  case (Direct_Insert_Costed_Base S x \<Delta> M_of t h k cap d B)
  then show ?case
    by (rule Charged_Direct_Insert_Base)
next
  ...
  show ?case
    by (rule Charged_Direct_Insert_Step
      [where D = D and c_insert = c_insert and a = a and betas = betas
        and bs = bs and charged_Us = Us
        and child_outputs =
          "range_tree_child_list (find_pivots_pivots_capped k cap d S B) a bs"
        and U_loop = U_loop and c_loop = c_loop
        and child_costs_loop = child_costs_loop])
      (use Direct_Insert_Costed_Step in auto)
qed

theorem charged_direct_insert_costed_partition_loop_state_child_call_charged_ranges:
  "charged_direct_insert_costed_partition_loop_state \<Delta> M_of t h k cap l d P B d' D a
      betas bs B' charged_Us child_outputs U c child_costs \<Longrightarrow>
   length child_costs = length bs \<and>
   length child_outputs = length bs \<and>
   list_all2
      ...
      child_costs child_outputs \<and>
   list_all2
      (\<lambda>c_child charged_child. \<exists>U_child S_child B_child d_child B_child'.
        charged_direct_insert_costed_bmssp \<Delta> M_of t h k cap l d S_child B_child
          d_child B_child' U_child c_child \<and>
        bmssp_pre_full d S_child B_child \<and>
        (\<forall>x\<in>S_child. reachable s x) \<and>
        card S_child \<le> M_of l \<and>
        (\<forall>x\<in>S_child. below_bound (d x) B_child))
      child_costs (range_tree_child_list P a bs) \<and>
   charged_Us = range_tree_chain_list P a bs B'"
and charged_direct_insert_costed_bmssp_child_call_charged_ranges_trivial:
...
next
  ...
  have card_pull: "card S_pull \<le> M_of l"
    using Charged_Direct_Insert_State_Step
  ...
  have head:
    "\<exists>S_child B_child d_child' B_child'.
      ...
      card S_child \<le> M_of l \<and>
      (\<forall>x\<in>S_child. below_bound (d x) B_child)"
    using Charged_Direct_Insert_State_Step card_pull below_pull by blast
  have head_charged:
    "\<exists>U_child' S_child B_child d_child' B_child'.
      charged_direct_insert_costed_bmssp \<Delta> M_of t h k cap l d S_child B_child
        d_child' B_child' U_child' c_child \<and>
      bmssp_pre_full d S_child B_child \<and>
      (\<forall>x\<in>S_child. reachable s x) \<and>
    ...
    using head by blast
  show ?case
    using Charged_Direct_Insert_State_Step head head_charged by simp
next
  case (Charged_Direct_Insert_Base S x \<Delta> M_of t h k cap d B)
  then show ?case
    by simp
next
  case (Charged_Direct_Insert_Step D k cap d S B c_insert t \<Delta> M_of h l d' a
      betas bs B' charged_Us child_outputs U_loop c_loop child_costs_loop U c)
  then show ?case
    by simp
qed

theorem charged_direct_insert_costed_partition_loop_state_trace:
  "charged_direct_insert_costed_partition_loop_state \<Delta> M_of t h k cap l d P B d' D a
     betas bs B' charged_Us child_outputs U c child_costs \<Longrightarrow>
   sound_label d \<Longrightarrow>
   bmssp_pre_full d P B \<Longrightarrow>
   (\<And>x. x \<in> P \<Longrightarrow> reachable s x) \<Longrightarrow>
   concrete_partition_loop_trace P B a bs d' B' charged_Us U"
and charged_direct_insert_costed_bmssp_correct:
  "charged_direct_insert_costed_bmssp \<Delta> M_of t h k cap l d S B d' B' U c \<Longrightarrow>
   sound_label d \<Longrightarrow>
...
proof (induction rule:
  ...
  have a_le: "bound_le (Fin a) B"
    using Charged_Direct_Insert_State_Done by blast
...
next
  ...
  have pre_beta: "bmssp_pre_full d P (Fin beta)"
    by (rule bmssp_pre_full_bound_mono
      [OF Charged_Direct_Insert_State_Step.prems(2)])
      (use Charged_Direct_Insert_State_Step.hyps in simp)
  ...
  have child_post_split:
    ...
    using child_post Charged_Direct_Insert_State_Step.hyps by simp
  have U_child_tree: "U_child = bound_tree P (Fin b)"
    using pull_recursive_post_lifts_bound_tree[OF cover_beta child_post_split] .
  have complete_child: "complete_on d_child U_child"
    using child_post unfolding bmssp_post_full_def by blast
  have complete_child':
    "complete_on d' U_child"
    using complete_child Charged_Direct_Insert_State_Step
    unfolding complete_preserved_def by metis
  have head_complete: "complete_on d' charged_child"
    by (rule complete_on_subset[OF complete_child'])
      (use U_child_tree Charged_Direct_Insert_State_Step
        range_tree_subset_bound_tree in metis)
  have tail_trace:
    "concrete_partition_loop_trace P B b bs d' B' charged_tail U_tail"
    using Charged_Direct_Insert_State_Step by metis
  then have tail_le: "bound_le B' B"
    and tail_mono: "nondecreasing_from b bs"
    and tail_bounds: "bounds_le B' (b # bs)"
    and tail_children:
      "list_all2 (\<lambda>U X. U = X \<and> complete_on d' U) charged_tail
  ...
  have lower_complete: "complete_on d' (bound_tree P (Fin a))"
    using Charged_Direct_Insert_State_Step by metis
  have charged_eq: "charged_child = range_tree P a (Fin b)"
    using Charged_Direct_Insert_State_Step by metis
  show ?case
    unfolding concrete_partition_loop_trace_def
    using tail_le tail_mono tail_bounds tail_children head_complete lower_complete charged_eq
      Charged_Direct_Insert_State_Step.hyps
    by simp
next
  case (Charged_Direct_Insert_Base S x \<Delta> M_of t h k cap d B)
  have post:
    "bmssp_post d S B
      ...
      (base_case_bound k x B)
      (base_case_vertices k x B)"
    using base_case_result_bmssp_post[OF Charged_Direct_Insert_Base.hyps,
...
next
  ...
  have sound_fp: "sound_label ?d_fp"
    ...
    by (rule fp_iter_capped_label_sound[OF Charged_Direct_Insert_Step.prems(1)
      Charged_Direct_Insert_Step.prems(3)])
  have pivot_pre: "bmssp_pre_full ?d_fp ?P B"
    using find_pivots_capped_establishes_pivot_pre_concrete
      [OF Charged_Direct_Insert_Step.prems(1) Charged_Direct_Insert_Step.prems(2)
        Charged_Direct_Insert_Step.prems(3)] .
  ...
  have P_reaches: "\<And>x. x \<in> ?P \<Longrightarrow> reachable s x"
    using P_subset_S Charged_Direct_Insert_Step.prems(3) by blast
  have trace:
    "concrete_partition_loop_trace ?P B a bs d' B' charged_Us U_loop"
    using Charged_Direct_Insert_Step.IH sound_fp pivot_pre P_reaches by blast
  ...
  proof -
    have "U_loop \<union> ?W = bound_tree S B'"
      by (rule concrete_capped_find_pivots_final_tree_assembly
        [OF Charged_Direct_Insert_Step.prems(1)
          Charged_Direct_Insert_Step.prems(2)
          Charged_Direct_Insert_Step.prems(3) loop_post])
    then show ?thesis
      using Charged_Direct_Insert_Step(6) by simp
  qed
  have loop_complete: "complete_on d' U_loop"
    using loop_post unfolding bmssp_post_full_def by blast
  have U_complete: "complete_on d' U"
    using complete_on_Un[OF loop_complete Charged_Direct_Insert_Step(5)]
      Charged_Direct_Insert_Step(6) by simp
  show ?case
    using le U_tree U_complete unfolding bmssp_post_full_def by blast
qed

lemma charged_direct_insert_costed_bmssp_step_with_insert_cost:
  assumes init:
      "D = label_partition_view
        (find_pivots_label_capped k cap d S B)
    ...
    and loop:
      "charged_direct_insert_costed_partition_loop_state \<Delta> M_of t h k cap l
        (find_pivots_label_capped k cap d S B)
        (find_pivots_pivots_capped k cap d S B) B d' D a betas bs B'
        charged_Us child_outputs U_loop c_loop child_costs_loop"
    and complete:
      "complete_on d'
  ...
  shows "charged_direct_insert_costed_bmssp \<Delta> M_of t h k cap (Suc l) d S B d' B' U c"
  using assms by (rule Charged_Direct_Insert_Step)

lemma charged_direct_insert_costed_bmssp_Suc_exists_from_initial_loop_at_bound:
  assumes loop:
      "\<exists>d' a betas bs charged_Us child_outputs U_loop c_loop child_costs_loop.
        charged_direct_insert_costed_partition_loop_state \<Delta> M_of t h k cap l
          (find_pivots_label_capped k cap d S B)
          (find_pivots_pivots_capped k cap d S B) B d'
          (label_partition_view
            (find_pivots_label_capped k cap d S B)
            (find_pivots_pivots_capped k cap d S B))
          a betas bs B' charged_Us child_outputs U_loop c_loop child_costs_loop \<and>
        complete_on d'
          {v \<in> bound_tree S B'.
            find_pivots_label_capped k cap d S B v = dist s v} \<and>
        partition_initial_insert_cost_bound c_insert t
          (find_pivots_pivots_capped k cap d S B)"
  shows "\<exists>d' U c.
    charged_direct_insert_costed_bmssp \<Delta> M_of t h k cap (Suc l) d S B d' B' U c"
proof -
  let ?d_fp = "find_pivots_label_capped k cap d S B"
  let ?P = "find_pivots_pivots_capped k cap d S B"
  let ?D = "label_partition_view ?d_fp ?P"
  obtain d' a betas bs charged_Us child_outputs U_loop c_loop
      child_costs_loop where
    loop_run:
      "charged_direct_insert_costed_partition_loop_state \<Delta> M_of t h k cap l
        ?d_fp ?P B d' ?D a betas bs B' charged_Us child_outputs U_loop c_loop
        child_costs_loop"
    and complete:
      "complete_on d' {v \<in> bound_tree S B'. ?d_fp v = dist s v}"
    and insert:
      "partition_initial_insert_cost_bound c_insert t ?P"
    using loop by blast
  let ?U = "U_loop \<union> {v \<in> bound_tree S B'. ?d_fp v = dist s v}"
  have run:
    "charged_direct_insert_costed_bmssp \<Delta> M_of t h k cap (Suc l) d S B d' B'
      ?U (fp_iter_capped_scan_cost k cap d S S B + c_insert + c_loop)"
    by (rule charged_direct_insert_costed_bmssp_step_with_insert_cost
      [OF refl insert loop_run complete refl refl])
  then show ?thesis
    by blast
qed

lemma charged_direct_insert_costed_bmssp_Suc_exists_from_initial_loop_at_bound_with_insert:
  assumes loop:
...
proof -
  obtain c_insert where
    ...
    using loop by blast
  then show ?thesis
    by (rule charged_direct_insert_costed_bmssp_Suc_exists_from_initial_loop_at_bound)
qed

lemma charged_direct_insert_costed_bmssp_Suc_exists_from_initial_loop:
  assumes loop:
      "\<exists>d' a betas bs B' charged_Us child_outputs U_loop c_loop child_costs_loop.
        charged_direct_insert_costed_partition_loop_state \<Delta> M_of t h k cap l
          (find_pivots_label_capped k cap d S B)
          (find_pivots_pivots_capped k cap d S B) B d'
          (label_partition_view
...
proof -
  ...
  let ?P = "find_pivots_pivots_capped k cap d S B"
  let ?D = "label_partition_view ?d_fp ?P"
  obtain d' a betas bs B' charged_Us child_outputs U_loop c_loop
      child_costs_loop where
    loop_run:
      "charged_direct_insert_costed_partition_loop_state \<Delta> M_of t h k cap l
        ?d_fp ?P B d' ?D a betas bs B' charged_Us child_outputs U_loop c_loop
...
lemma charged_direct_insert_partition_loop_state_done_exists:
  assumes empty: "keys_of D = {}"
    and a_bound: "bound_le (Fin a) B"
    and complete: "complete_on d' (bound_tree P B)"
  shows "\<exists>betas bs B' charged_Us child_outputs U c child_costs.
    charged_direct_insert_costed_partition_loop_state \<Delta> M_of t h k cap l
      d P B d' D a betas bs B' charged_Us child_outputs U c child_costs"
proof -
  have run:
    "charged_direct_insert_costed_partition_loop_state \<Delta> M_of t h k cap l
      d P B d' D a [] [] B [range_tree P a B] []
      (bound_tree P (Fin a) \<union> \<Union>(set [range_tree P a B])) 0 []"
    by (rule Charged_Direct_Insert_State_Done[OF empty a_bound complete])
  then show ?thesis
    by blast
qed

lemma charged_direct_insert_partition_loop_state_stop_exists:
...
proof -
  have run:
    "charged_direct_insert_costed_partition_loop_state \<Delta> M_of t h k cap l
      d P B d' D a [] [] (Fin a) [range_tree P a (Fin a)] []
      (bound_tree P (Fin a) \<union> \<Union>(set [range_tree P a (Fin a)])) 0 []"
    by (rule Charged_Direct_Insert_State_Stop[OF a_bound complete threshold])
  then show ?thesis
    by blast
qed

lemma charged_direct_insert_partition_loop_state_step_exists:
  assumes pull: "pull_separates D (M_of l) Bmax S_pull beta D_pull"
    and beta_bound: "bound_le (Fin beta) B"
  ...
  shows "\<exists>c child_costs.
    charged_direct_insert_costed_partition_loop_state \<Delta> M_of t h k cap l
      d P B d' D a (beta # betas) (b # bs) B'
      (range_tree P a (Fin b) # charged_tail)
      (U_child # child_outputs_tail)
      (bound_tree P (Fin a) \<union>
        \<Union>(set (range_tree P a (Fin b) # charged_tail))) c child_costs"
proof -
  let ?charged_child = "range_tree P a (Fin b)"
  let ?direct = "edge_relaxation_pairs_in_bound d_child ?charged_child beta B"
  let ?lower = "edge_relaxation_pairs_between d_child ?charged_child b beta"
  let ?sources = "label_pairs_between d S_pull b beta"
  let ?batch = "?direct @ ?lower @ ?sources"
  have pull_cost: "partition_pull_cost_bound 0 S_pull"
    unfolding partition_pull_cost_bound_def by simp
  have direct_cost: "partition_batch_cost_bound 0 t ?direct"
    unfolding partition_batch_cost_bound_def by simp
  have lower_cost: "partition_batch_cost_bound 0 h ?lower"
    unfolding partition_batch_cost_bound_def by simp
  have source_cost: "partition_batch_cost_bound 0 h ?sources"
    unfolding partition_batch_cost_bound_def by simp
  have run:
    "charged_direct_insert_costed_partition_loop_state \<Delta> M_of t h k cap l
      d P B d' D a (beta # betas) (b # bs) B'
      (?charged_child # charged_tail) (U_child # child_outputs_tail)
      (bound_tree P (Fin a) \<union> \<Union>(set (?charged_child # charged_tail)))
      (0 + 0 + 0 + 0 + c_child + c_tail) (c_child # child_costs_tail)"
    by (rule Charged_Direct_Insert_State_Step
      [where Bmax = Bmax and S_pull = S_pull and D_pull = D_pull
        and charged_child = ?charged_child and direct_edge_batch = ?direct
        and lower_edge_batch = ?lower and source_batch = ?sources
        and batch = ?batch and D_next = "batch_min_update D_pull ?batch"
        and c_pull = 0 and c_direct = 0 and c_lower = 0 and c_sources = 0
        and c_child = c_child and c_tail = c_tail])
      (use pull beta_bound child_pre S_pull_def S_pull_reaches a_le_b
        a_bound complete_prefix below_threshold child preserved tail
        pull_cost direct_cost lower_cost source_cost in simp_all)
  then show ?thesis
    by blast
qed

lemma charged_direct_insert_partition_loop_state_cases_exists_with_complete:
      ...
      "(keys_of D = {} \<and>
       ...
       (\<exists>S_pull beta D_pull B'.
        ...
        (\<exists>d_child b U_child c_child betas bs charged_tail
          ...
          complete_preserved d_child d' U_child \<and>
          charged_direct_insert_costed_partition_loop_state \<Delta> M_of t h k cap l
            d P B d'
            (batch_min_update D_pull
  ...
  shows "\<exists>betas bs B' charged_Us child_outputs U c child_costs.
    charged_direct_insert_costed_partition_loop_state \<Delta> M_of t h k cap l
      d P B d' D a betas bs B' charged_Us child_outputs U c child_costs \<and>
    complete_on d' (W B')"
proof -
  consider
    (Done_Case) "keys_of D = {}"
  ...
  | (Step_Case) S_pull beta D_pull B' where
      "pull_separates D (M_of l) Bmax S_pull beta D_pull"
      "bound_le (Fin beta) B"
      "bmssp_pre_full d S_pull (Fin beta)"
      "S_pull = split_below d P beta"
      "\<forall>x\<in>S_pull. reachable s x"
      "complete_on d' (bound_tree P (Fin a))"
      "card (bound_tree P (Fin a)) < k * cap"
      "complete_on d' (W B')"
      "\<exists>d_child b U_child c_child betas bs charged_tail child_outputs_tail
          U_tail c_tail child_costs_tail.
        a \<le> b \<and>
        bound_le (Fin a) B' \<and>
        charged_direct_insert_costed_bmssp \<Delta> M_of t h k cap l d S_pull
          (Fin beta) d_child (Fin b) U_child c_child \<and>
        complete_preserved d_child d' U_child \<and>
        charged_direct_insert_costed_partition_loop_state \<Delta> M_of t h k cap l
          d P B d'
          (batch_min_update D_pull
  ...
  proof cases
    ...
    have run:
      "charged_direct_insert_costed_partition_loop_state \<Delta> M_of t h k cap l
        d P B d' D a [] [] B [range_tree P a B] []
        (bound_tree P (Fin a) \<union> \<Union>(set [range_tree P a B])) 0 []"
      by (rule Charged_Direct_Insert_State_Done[OF Done_Case(1-3)])
    show ?thesis
      using run Done_Case(4) by blast
  next
    case Stop_Case
    have run:
      "charged_direct_insert_costed_partition_loop_state \<Delta> M_of t h k cap l
        d P B d' D a [] [] (Fin a) [range_tree P a (Fin a)] []
        (bound_tree P (Fin a) \<union> \<Union>(set [range_tree P a (Fin a)])) 0 []"
      by (rule Charged_Direct_Insert_State_Stop[OF Stop_Case(1-3)])
    show ?thesis
      using run Stop_Case(4) by blast
  next
    case (Step_Case S_pull beta D_pull B')
    then obtain d_child b U_child c_child betas bs charged_tail
        child_outputs_tail U_tail c_tail child_costs_tail where
      a_le_b: "a \<le> b"
      and a_bound: "bound_le (Fin a) B'"
      and child:
        "charged_direct_insert_costed_bmssp \<Delta> M_of t h k cap l d S_pull
    ...
    have run:
      "\<exists>c child_costs.
        charged_direct_insert_costed_partition_loop_state \<Delta> M_of t h k cap l
          ...
          (bound_tree P (Fin a) \<union>
            \<Union>(set (range_tree P a (Fin b) # charged_tail))) c child_costs"
      by (rule charged_direct_insert_partition_loop_state_step_exists
        [OF Step_Case(1) Step_Case(2) Step_Case(3) Step_Case(4)
          Step_Case(5) a_le_b a_bound Step_Case(6) Step_Case(7)
          child preserved tail])
    then show ?thesis
      using Step_Case(8) by blast
  qed
qed

lemma charged_direct_insert_costed_bmssp_Suc_exists_if_pivots_empty_same_bound:
  assumes a_bound: "bound_le (Fin a) B"
    and pivots_empty:
      "find_pivots_pivots_capped k cap d S B = {}"
  shows "\<exists>d' U c.
    charged_direct_insert_costed_bmssp \<Delta> M_of t h k cap (Suc l) d S B
      d' B U c"
proof -
  let ?d_fp = "find_pivots_label_capped k cap d S B"
  let ?P = "find_pivots_pivots_capped k cap d S B"
  ...
  have empty: "keys_of ?D = {}"
    using pivots_empty unfolding label_partition_view_def by simp
  have loop:
    "charged_direct_insert_costed_partition_loop_state \<Delta> M_of t h k cap l
      ?d_fp ?P B (dist s) ?D a [] [] B [range_tree ?P a B] []
      ?U_loop 0 []"
    by (rule Charged_Direct_Insert_State_Done)
      (use empty a_bound in \<open>simp_all add: complete_on_def\<close>)
  have complete:
    "complete_on (dist s)
      {v \<in> bound_tree S B. ?d_fp v = dist s v}"
    unfolding complete_on_def by simp
  have insert: "partition_initial_insert_cost_bound 0 t ?P"
    unfolding partition_initial_insert_cost_bound_def by simp
  let ?U = "?U_loop \<union> {v \<in> bound_tree S B. ?d_fp v = dist s v}"
  have run:
    "charged_direct_insert_costed_bmssp \<Delta> M_of t h k cap (Suc l) d S B
      (dist s) B ?U
      (fp_iter_capped_scan_cost k cap d S S B + 0 + 0)"
    by (rule charged_direct_insert_costed_bmssp_step_with_insert_cost
      [OF refl insert loop complete refl refl])
  then show ?thesis
    by blast
qed

lemma charged_direct_insert_costed_bmssp_Suc_exists_if_initial_threshold_bound:
  assumes a_bound: "bound_le (Fin a) B"
    and threshold:
      "k * cap \<le>
...
proof -
  ...
  have loop:
    ...
    by (rule Charged_Direct_Insert_State_Stop)
      (use a_bound threshold in \<open>simp_all add: complete_on_def\<close>)
  have complete:
    "complete_on (dist s)
...
theorem finite_initial_label_charged_direct_insert_costed_top_level_correct:
  assumes all_reachable: "\<And>v. v \<in> V \<Longrightarrow> reachable s v"
    and run:
      "charged_direct_insert_costed_bmssp \<Delta> M_of t h k cap l
...
theorem finite_initial_label_charged_direct_insert_costed_top_level_reachable_correct:
  assumes run:
      "charged_direct_insert_costed_bmssp \<Delta> M_of t h k cap l
        finite_initial_label {s} Infinity d' Infinity U c"
  shows "sssp_correct d'"
...
inductive_cases charged_direct_insert_costed_bmssp_zeroE:
  "charged_direct_insert_costed_bmssp \<Delta> M_of t h k cap 0 d S B d' B' U c"

inductive_cases charged_direct_insert_costed_bmssp_SucE:
  "charged_direct_insert_costed_bmssp \<Delta> M_of t h k cap (Suc l) d S B d' B' U c"
...
lemma charged_direct_insert_costed_partition_loop_state_mono:
  assumes run:
    "charged_direct_insert_costed_partition_loop_state \<Delta> M_of t h k cap l d P B d' D a
...
lemma charged_direct_insert_costed_partition_loop_state_beta_bounds:
  assumes run:
    "charged_direct_insert_costed_partition_loop_state \<Delta> M_of t h k cap l d P B d' D a
      betas bs B' charged_Us child_outputs U c child_costs"
  shows "bounds_le B betas"
...
theorem charged_direct_insert_costed_partition_loop_state_success_or_threshold:
  "charged_direct_insert_costed_partition_loop_state \<Delta> M_of t h k cap l d P B d' D a
      betas bs B' charged_Us child_outputs U c child_costs \<Longrightarrow>
   sound_label d \<Longrightarrow>
   bmssp_pre_full d P B \<Longrightarrow>
   (\<And>x. x \<in> P \<Longrightarrow> reachable s x) \<Longrightarrow>
   B' = B \<or> k * cap \<le> card U"
and charged_direct_insert_costed_bmssp_success_or_threshold_trivial:
  "charged_direct_insert_costed_bmssp \<Delta> M_of t h k cap l d S B d' B' U c \<Longrightarrow> True"
proof (induction rule:
    charged_direct_insert_costed_partition_loop_state_charged_direct_insert_costed_bmssp.inducts)
  case (Charged_Direct_Insert_State_Done D a B d' P \<Delta> M_of t h k cap l d)
...
next
  case (Charged_Direct_Insert_State_Step D M_of l Bmax S_pull beta D_pull B d
      P a b B_out d_out k cap \<Delta> t h d_child U_child c_child
      charged_child direct_edge_batch lower_edge_batch source_batch batch D_next
      c_pull c_direct c_lower c_sources betas bs charged_tail
      child_outputs_tail U_tail c_tail child_costs_tail c)
  have tail:
    "B_out = B \<or> k * cap \<le> card U_tail"
    using Charged_Direct_Insert_State_Step.IH
      Charged_Direct_Insert_State_Step.prems by blast
  then show ?case
  proof
    assume "B_out = B"
    then show ?thesis
  ...
  next
    assume threshold_tail: "k * cap \<le> card U_tail"
    have tail_trace:
      "concrete_partition_loop_trace P B b bs d_out B_out charged_tail U_tail"
      by (rule charged_direct_insert_costed_partition_loop_state_trace)
        (use Charged_Direct_Insert_State_Step in blast)+
    have U_tail_def:
      "U_tail = bound_tree P (Fin b) \<union> \<Union>(set charged_tail)"
      using tail_trace unfolding concrete_partition_loop_trace_def by blast
    ...
    have split_b:
      "bound_tree P (Fin b) =
        bound_tree P (Fin a) \<union> range_tree P a (Fin b)"
      using bound_tree_split_at[of a "Fin b" P] Charged_Direct_Insert_State_Step
      by simp
    have U_eq_tail:
      "bound_tree P (Fin a) \<union>
          \<Union>(set (charged_child # charged_tail)) =
        U_tail"
      using U_tail_def split_b charged_child_eq by auto
    show ?thesis
      using threshold_tail U_eq_tail by simp
  qed
next
...
theorem charged_direct_insert_costed_partition_loop_state_cost_bound_by_child_sources_and_edge_ranges:
  "charged_direct_insert_costed_partition_loop_state \<Delta> M_of t h k cap l d P B d' D a
      betas bs B' charged_Us child_outputs U c child_costs \<Longrightarrow>
   sound_label d \<Longrightarrow>
   bmssp_pre_full d P B \<Longrightarrow>
   ...
   \<exists>child_sources.
    ...
    c \<le> sum_list child_costs + sum_list (map card child_sources) +
      ...
      h * sum_list
        (map card (range_tree_child_edge_range_list P a betas bs)) +
      h * sum_list (map card child_sources)"
and charged_direct_insert_costed_bmssp_cost_child_sources_and_edge_ranges_trivial:
  "charged_direct_insert_costed_bmssp \<Delta> M_of t h k cap l d S B d' B' U c \<Longrightarrow> True"
proof (induction rule:
    charged_direct_insert_costed_partition_loop_state_charged_direct_insert_costed_bmssp.inducts)
  case (Charged_Direct_Insert_State_Done D a B d' P \<Delta> M_of t h k cap l d)
...
next
    ...
    and tail_cost:
      "c_tail \<le>
        ...
        h * sum_list
          (map card (range_tree_child_edge_range_list P b betas bs)) +
        h * sum_list (map card child_sources_tail)"
    using Charged_Direct_Insert_State_Step.IH Charged_Direct_Insert_State_Step.prems by blast
  have card_pull: "card S_pull \<le> M_of l"
    using Charged_Direct_Insert_State_Step
    unfolding pull_separates_def by blast
  have below_pull: "\<forall>x\<in>S_pull. below_bound (d x) (Fin beta)"
  ...
  have child_post:
    "bmssp_post_full d S_pull (Fin beta) d_child (Fin b) U_child"
    by (rule charged_direct_insert_costed_bmssp_correct
      [OF child_run Charged_Direct_Insert_State_Step.prems(1) child_pre child_reaches])
  have child_complete: "complete_on d_child U_child"
    using child_post unfolding bmssp_post_full_def by blast
  have pre_beta: "bmssp_pre_full d P (Fin beta)"
    by (rule bmssp_pre_full_bound_mono
  ...
  have cover_beta: "complete_tree_cover d P (Fin beta)"
    using bmssp_pre_full_complete_tree_cover[OF pre_beta] .
  have child_post_split:
    "bmssp_post_full d (split_below d P beta) (Fin beta) d_child
      (Fin b) U_child"
    using child_post Charged_Direct_Insert_State_Step.hyps by simp
  have U_child_tree: "U_child = bound_tree P (Fin b)"
    using pull_recursive_post_lifts_bound_tree[OF cover_beta child_post_split] .
  have charged_child_eq: "charged_child = range_tree P a (Fin b)"
    using Charged_Direct_Insert_State_Step by blast
  have charged_complete: "complete_on d_child charged_child"
    by (rule complete_on_subset[OF child_complete])
      (use U_child_tree charged_child_eq range_tree_subset_bound_tree in metis)
  have charged_reaches: "\<And>u. u \<in> charged_child \<Longrightarrow> reachable s u"
    unfolding charged_child_eq range_tree_def tree_set_def tree_path_def by blast
  have direct_cost:
    "c_direct \<le> t * card (outgoing_edges_range charged_child beta B)"
    by (rule partition_batch_cost_bound_edge_relaxation_pairs_in_bound
      [where d = d_child])
      (use Charged_Direct_Insert_State_Step charged_complete charged_reaches in auto)
  have lower_source_cost:
    "c_lower + c_sources \<le>
      h * card (outgoing_edges_range charged_child b (Fin beta)) +
      h * card S_pull"
    by (rule range_costed_split_batch_cost_le_range_card[
        where d = d and S = S_pull and d_child = d_child and b = b
          and beta = beta])
  ...
  have len: "length ?child_sources = length (b # bs)"
    using len_tail by simp
  have head_source:
    "\<exists>c_child' U_child' B_child d_child' B_child'.
      charged_direct_insert_costed_bmssp \<Delta> M_of t h k cap l d S_pull B_child
        d_child' B_child' U_child' c_child' \<and>
      bmssp_pre_full d S_pull B_child \<and>
      (\<forall>x\<in>S_pull. reachable s x) \<and>
      card S_pull \<le> M_of l \<and>
  ...
  have sources:
    "list_all2
      (\<lambda>S_child charged_child. \<exists>c_child U_child B_child d_child B_child'.
        ...
        (\<forall>x\<in>S_child. below_bound (d x) B_child))
      ?child_sources (range_tree_child_list P a (b # bs))"
    using head_source sources_tail Charged_Direct_Insert_State_Step by simp
  have cost:
    "c \<le> sum_list (c_child # child_costs_tail) +
      sum_list (map card ?child_sources) +
      t * sum_list
  ...
  proof -
    ...
    have "c_pull + c_direct + c_lower + c_sources + c_child + c_tail \<le>
        card S_pull +
        t * card (outgoing_edges_range charged_child beta B) +
        (h * card (outgoing_edges_range charged_child b (Fin beta)) +
          h * card S_pull) +
        c_child +
        (sum_list child_costs_tail + sum_list (map card child_sources_tail) +
    ...
    also have "\<dots> =
        ...
        h * sum_list
          (map card
            (range_tree_child_edge_range_list P a (beta # betas) (b # bs))) +
        h * sum_list (map card ?child_sources)"
      using charged_child_eq by simp
    finally show ?thesis
      using c_eq by simp
  qed
  show ?case
...
theorem charged_direct_insert_costed_partition_loop_state_child_cost_bounds_with_invariants:
  "charged_direct_insert_costed_partition_loop_state \<Delta> M_of t h k cap l d P B d' D a
      betas bs B' charged_Us child_outputs U c child_costs \<Longrightarrow>
    P \<subseteq> V \<Longrightarrow>
    k * card P \<le> cap \<Longrightarrow>
    ...
    (\<And>c_child charged_child U_child S_child B_child d_child B_child'.
        \<lbrakk>charged_direct_insert_costed_bmssp \<Delta> M_of t h k cap l d S_child B_child
          ...
          k * card S_child \<le> cap;
          tree_antichain S_child\<rbrakk>
        \<Longrightarrow> c_child \<le> level_range_cost_bound A R L charged_child) \<Longrightarrow>
    list_all2 (\<lambda>c_child charged_child.
      c_child \<le> level_range_cost_bound A R L charged_child)
    child_costs (range_tree_child_list P a bs)"
and charged_direct_insert_costed_bmssp_child_cost_bounds_with_invariants_trivial:
  "charged_direct_insert_costed_bmssp \<Delta> M_of t h k cap l d S B d' B' U c \<Longrightarrow> True"
proof (induction rule:
    charged_direct_insert_costed_partition_loop_state_charged_direct_insert_costed_bmssp.inducts)
  case (Charged_Direct_Insert_State_Done D a B d' P \<Delta> M_of t h k cap l d)
    ...
    then show ?thesis
      using split_below_scaled_card_le
        [OF Charged_Direct_Insert_State_Step.prems(1)
          Charged_Direct_Insert_State_Step.prems(2), of d beta]
      by simp
  qed
  have pull_anti: "tree_antichain S_pull"
  proof -
    ...
    then show ?thesis
      using split_below_tree_antichain
        [OF Charged_Direct_Insert_State_Step.prems(3), of d beta]
      by simp
  qed
  have child_run:
    "charged_direct_insert_costed_bmssp \<Delta> M_of t h k cap l d S_pull
  ...
  have head:
    ...
    by (rule Charged_Direct_Insert_State_Step.prems(4)
      [OF child_run pre_pull reaches_pull card_pull below_pull
        pull_k_cap pull_anti])
  show ?case
    using head tail Charged_Direct_Insert_State_Step by simp
next
  case (Charged_Direct_Insert_Base S x \<Delta> M_of t h k cap d B)
  then show ?case
    by simp
...
theorem charged_direct_insert_costed_partition_loop_state_closes_level_bound_from_child_bound_with_invariants_and_edge_ranges:
  assumes run:
    "charged_direct_insert_costed_partition_loop_state \<Delta> M_of t h k cap l d P B d' D a
      betas bs B' charged_Us child_outputs U c child_costs"
    and sound: "sound_label d"
