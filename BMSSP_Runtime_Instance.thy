...
text \<open>
  The strengthened witness.  Re-exporting
  \<open>verified_bmssp_runtime_bigo_target\<close> on the same concrete path instance
  yields a closed, assumption-free statement that couples the
  \<open>O(m * (ln n) powr (2 / 3))\<close> running-time bound with the guarantee that the
  bounded running time is, for all but finitely many sizes, the cost of an
  explicit run returning the exact single-source shortest-path distances on the
  whole vertex set.  This certifies that the runtime headline is non-vacuous in
  the strong sense: the cost it bounds belongs to genuinely verified-correct
  BMSSP runs, not merely to an abstract least-cost integer.
\<close>

lemmas path_verified_runtime_bigo_target =
  pbr.runtime_headline.verified_bmssp_runtime_bigo_target
